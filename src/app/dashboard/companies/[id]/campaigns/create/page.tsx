...
export default function CreateCampaignPage({
  ...
  const isSubmitting =
    loadingState.uploadingCsv || loadingState.creatingCampaign;

  useEffect(() => {
    const initializeComponent = async () => {
      const resolvedParams = await params;
      setCompanyId(resolvedParams.id);
      fetchCompanyDetails(resolvedParams.id);
    };

    initializeComponent();
  }, [params]);

  const fetchCompanyDetails = async (id: string): Promise<void> => {
    try {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      ...
      router.push(
        `/dashboard/companies/${companyId}/campaigns/${newCampaign.campaign._id}`
      );
    } catch (err) {
      const errorMessage =
      ...
      <div className="max-w-4xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">
            Create VAPI AI Campaign
          </h1>
          <p className="text-gray-600 text-lg">
            Create a new VAPI AI campaign for{" "}
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-100 bg-gray-50">
              ...
              <p className="text-sm text-gray-600 mt-1">
                Upload your contact data as a CSV file or provide a URL
              </p>
            </div>
